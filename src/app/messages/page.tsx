...
export default function MessagesPage() {
  ...
  const showChat = !!selectedUsername;
  ...
  const sendMessage = async () => {
    ...
    if (newMsg) {
      ...
      setConversations((prev) => {
        ...
        if (exists) {
          return [{ ...exists, lastMessage: content, lastMessageAt: newMsg.created_at }, ...prev.filter((c) => c.userId !== otherUser.id)];
        }
  ...
  const ConversationList = ({ onClose }: { onClose?: () => void }) => (
      ...
      <div className="flex-1 overflow-y-auto px-2">
        {loading ? (
          ...
          filteredConversations.map((conv) => (
            <button
              key={conv.userId}
              onClick={() => { setSelectedUsername(conv.username); onClose?.(); }}
  ...
  const ChatPanel = () => (
    <>
      {!selectedUsername ? (
        ...
        <>
          {/* Chat header */}
          <div className="flex items-center gap-3 px-4 lg:px-6 py-4 shrink-0" style={{ borderBottom: "1px solid var(--border)" }}>
            {/* Back button — mobile only */}
            <button
              onClick={() => setSelectedUsername(null)}
              className="lg:hidden p-1.5 -ml-1 rounded-lg hover:opacity-70 transition-opacity"
          ...
          {/* Input */}
          <div className="px-4 lg:px-6 py-4 shrink-0" style={{ borderTop: "1px solid var(--border)" }}>
  ...
  return (
    <div className="h-[calc(100vh-56px)]" style={{ background: "var(--bg)" }}>
